      ...
      columns: [
        {
          ...
          render: (row) => (
            <span>
              {users &&
                `${users?.filter((x) => x._id == row.user)[0]?.prenom} ${
                  users?.filter((x) => x._id == row.user)[0]?.nom
                }`}
            </span>
          ),
        },
        { title: "Rôle", field: "role", lookup: NominationTypes },
        {
          ...
          render: (row) => (
            <span>
              {(units
                ? units?.filter((x) => x._id === row.unit)[0]?.nom
                : null) ??
                (groups
                  ? `Groupe ${
                      groups?.filter((x) => x._id === row.group)[0]?.numero
                    } ${groups.filter((x) => x._id === row.group)[0]?.nom}`
                  : null)}
            </span>
          ),
          editable: "never",
        },
        {
          ...
          field: "_id",
          render: (row) => (
            <span>{dateFromObjectId(row._id).toISOString()}</span>
          ),
        },
      ],
      data: nominations,
  ...
  const FetchNominations = async () => {
    try {
      if (Permissions(PermissionTypes.AddNomination, authedUser)) {
        const demandesNomination = await nominationClient.getNominations(
          past ? "complete" : "pending"
        );
        setNominations(demandesNomination);
      } else {
        const demandesNomination =
          await nominationClient.getPendingRecommendationForUser(authedUser._id);
        setNominations(demandesNomination);
      }
    } catch (error) {
  ...
  async function FetchMemberUnits() {
    try {
      var data = await unitClient.getMultipleById(
        nominations?.filter((x) => x.unit !== "").map((x) => x.unit)
      );
      if (data !== null) {
        setUnits(data);
      }
  ...
  async function FetchGroups() {
    try {
      var data = await groupClient.getMultipleById(
        nominations?.filter((x) => x.group !== "").map((x) => x.group)
      );
      if (data !== null) {
        setGroups(data);
      }
        ...
        <MaterialTable
          ...
          actions={[
            (rowData) => ({
              icon: "check",
              tooltip:
                "Compléter la nomination (ajoute la nomination en date du jour au dossier du membre, avec toutes les permissions reliées)",
            ...
            (rowData) => ({
              icon: "delete",
              tooltip:
                "Marque la nomination comme complétée, sans l'ajouter au dossier du membre",
          ...
          detailPanel={(rowData) => (
            <NominationRowDetail nomination={rowData} />
          )}
          onRowClick={(event, rowData) =>
            navigate("/app/membre/" + rowData.user)
          }
        />
      }
    </div>

...
const FormationResume = ({
  ...
  const article = data?.allMarkdownRemark?.edges.filter(
    (x) =>
      x?.node?.frontmatter?.niveau.toLowerCase() == niveau.toLowerCase() &&
      x.node.frontmatter.branche.toLowerCase().includes(branche.toLowerCase()),
  )[0]?.node;

  if (!article) {
    return <div>Article introuvé</div>;
  }

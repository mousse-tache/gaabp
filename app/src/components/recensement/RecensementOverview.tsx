...
const RecensementOverview = () => {
  ...
  const cols = [
    ...
    {
      title: "Date de soumission du recensement",
      field: "date",
      editable: false,
    },
    {
      title: "Coût total",
      field: "details.cost.totalPrice",
      render: (row) => `${row.details.cost.totalPrice}$`,
    },
  ...
  const FetchRecensements = async () => {
    try {
      const data = await recensementClient.getByPayment(paid);

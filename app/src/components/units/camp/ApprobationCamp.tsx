...
const ApprobationCamp = (): React.ReactElement => {
  ...
  const GetCamps = async () => {
    await setInit(false);
    const data = await campClient.getList();
    if (data) {
      setCamps(data);
    }
  ...
  const AutorizeCamp = async (camp) => {
    await campClient.save({
      ...camp,
      approuvePar: {
        id: authedUser._id,
        nom: `${authedUser.prenom} ${authedUser.nom}`,
      },
      approuve: true,
    });
    GetCamps();
  ...
  const cols = [
    {
      ...
      field: "unitInfo.nom",
      render: (row) => (
        <a href={`/app/unite/${row.unit}`} rel="noreferrer" target="_blank">
          {row?.unitInfo?.nom}
        </a>
      ),
      editable: false,
    ...
    {
      title: "URL de l'évaluation (https://...)",
      field: "evaluation",
      render: (row) => {
        return (
          <a href={row.evaluation} rel="noreferrer" target="_blank">
            {row.evaluation}
          </a>
        );
      },
  ...
  return (
    <Card>
      <Alert severity="info">
        Les évaluations de camp doivent être conduites selon le formulaire
        officiel d'évaluation.
      </Alert>
      <MaterialTable
        title="Camps"
        isLoading={!init}
        ...
        detailPanel={[
          {
            ...
            icon: "list",
            render: (rowData) => {
              return <CampSummary camp={rowData} />;
            },
          },
        ]}
        editable={{
